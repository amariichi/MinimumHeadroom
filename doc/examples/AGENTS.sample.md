...
- When spawning a helper, pass `permission_preset` to auto-configure tool permissions. Values: `reviewer` (read-only tools), `implementer` (read + edit), `full` (all tools). Also pass `agent_cmd` (`claude`, `agy`, or `codex`) so the runtime knows which config format to write.
