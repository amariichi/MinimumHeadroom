...
- **Keep `agent_id` stable** on every `face_ping` / `face_event` / `face_say` payload. Prefer setting `MH_FACE_AGENT_ID` in the agent process environment so the MCP server auto-fills it. Use `"__operator__"` only for the user-facing operator pane; use the assigned helper id (e.g. `"helper-1"`) for helpers. Do not hard-code `"__operator__"` when running as a helper. Without the correct `agent_id`, the visible 3D head may stop animating its mouth even though the text bubble and audio still arrive. `session_id` does not substitute for `agent_id`.
...
- Capture `active_stream_id` from `agent.list(scope="stream")`. Helpers for the current user task stay in that stream even when they work in another repository; omit `stream_id` to use the active default or pass that exact value.
- For cross-repository missions, pass explicit repository fields, use absolute `target_paths`, require `visible_in_active_stream=true`, and verify the helper appears in a fresh stream-scoped list. Treat a visibility warning as `needs_attention` unless separation is intentional.
