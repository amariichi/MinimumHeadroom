...
The default keeps `MH_QWEN_TTS_SPEED=1.0` so Qwen3 uses its unstretched output. If it feels slightly slow, adjust `MH_QWEN_TTS_SPEED` manually (for example `1.0`, `1.10`, or `1.20`). Values above `1.0` speak faster, but the extra time-stretch can slightly reduce clarity.
...
既定値は `MH_QWEN_TTS_SPEED=1.0` とし、Qwen3 の生の音声をそのまま使います。少し遅く感じる場合は、`MH_QWEN_TTS_SPEED` を手動で調整してください（例: `1.0`, `1.10`, `1.20`）。`1.0` より大きい値は、より速く話しますが、time-stretch のぶんだけ明瞭さが少し落ちる可能性があります。
