...
echo "[run-operator-stack] TTS_ENGINE=${TTS_ENGINE}"
...
if [[ "${TTS_ENGINE,,}" == "supertonic" ]]; then
  echo "[run-operator-stack] Supertonic voice=${MH_SUPERTONIC_VOICE:-M1} language=${MH_SUPERTONIC_LANGUAGE:-auto} steps=${MH_SUPERTONIC_STEPS:-8} speed=${MH_SUPERTONIC_SPEED:-1.05}"
fi
...
start_proc "face-app" \
  ...
  MH_SUPERTONIC_VOICE="${MH_SUPERTONIC_VOICE:-}" \
  MH_SUPERTONIC_LANGUAGE="${MH_SUPERTONIC_LANGUAGE:-}" \
  MH_SUPERTONIC_STEPS="${MH_SUPERTONIC_STEPS:-}" \
  MH_SUPERTONIC_SPEED="${MH_SUPERTONIC_SPEED:-}" \
  MH_SUPERTONIC_INTRA_OP_THREADS="${MH_SUPERTONIC_INTRA_OP_THREADS:-}" \
  MH_SUPERTONIC_INTER_OP_THREADS="${MH_SUPERTONIC_INTER_OP_THREADS:-}" \
  ...
  MH_OPERATOR_FACE_AGENT_ID="$MH_OPERATOR_FACE_AGENT_ID" \
  MH_OPERATOR_FACE_AGENT_LABEL="$MH_OPERATOR_FACE_AGENT_LABEL" \
